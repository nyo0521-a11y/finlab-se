...
| 担当 | やったこと |
|---|---|
| 私 | 「有名ブログのようなポップで洗練されたトップページにしたい」「Hero・ツール紹介・カテゴリ・最新記事の4ブロック構成で」といった要望を日本語で伝えるだけ |
| Claude Code | 要望をデザイン指示に翻訳し、Claude in Chrome経由でClaude Designを操作してモックアップを生成 |
| Claude Design | 指示に沿ったWebページのモックアップを生成 |

そこから「このカードのデザインをもう少しシンプルに」「フォントの重さを変えて」といった修正も、すべてClaude Codeに伝えるだけで、ブラウザ側の操作は自動で進みます。ここで大切なのは、<strong>気に入った部分と気に入らない部分を言葉にすること</strong>だけ。コードの知識も、Claude Designの使い方の学習も不要でした。
