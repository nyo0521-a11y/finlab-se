...
なお、Hugoとは「Markdownファイル（.md）を書くと、自動的にHTMLのWebサイトに変換してくれるツール」ということで、Claude Codeでブログを記載するとMarkdownファイルというテキストファイルができるのですが、それをブログの形にしてくれる変換ツールのようなイメージです。
...
そして、次回以降は、ブログのテキスト作成からサーバへのアップロードまで自動的にやってくれました。
また、途中でサムネイル画像を入れたいと思い、そちらもサイズ等を指示して入れるようにしました。
