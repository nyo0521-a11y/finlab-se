...
そこで導入したのが**投稿キュー（投稿待ちリスト）**<!-- -->という概念です。投稿する記事の場所を列挙した管理ファイル（投稿待ちリスト）を用意し、Cloudflare Workersに呼び出されたGitHub Actionsがその先頭から1件ずつ取り出して投稿する、という先入れ先出し方式です。
...
このブログでは「予約投稿」の機能があります。記事ファイルの冒頭に公開日時を書いておくと、その日時まで本番サイトに表示されない仕組みです（毎時0分の自動ビルドで公開）。このブログの公開フローについては[別記事で詳しく解説しています](/posts/cloudflare-pages-blog-publish-flow/)。

この予約投稿とX自動投稿を組み合わせると、**タイミングの問題**<!-- -->が発生します。ブログを本番公開フォルダ（GitHubの`posts`フォルダ）にアップロードした瞬間にXキューへ登録してしまうと、記事が実際に公開される前にX投稿が発火するリスクがあります。
