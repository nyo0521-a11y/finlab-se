...
description: "コードを1行も書かずに、Claude Codeとの対話だけで投資シミュレーターとブログを2週間で立ち上げた体験記。Hugo×Cloudflare Pagesの構成、非エンジニアでも進められた手順、タイムゾーンや認証エラーなどハマったポイント3選まで、金融SEが正直に振り返ります。"
showToc: true
cover:
...
「Claude Codeって便利らしいけれど、自分に何ができるのか正直イメージがわかない」——AIコーディングというキーワードを耳にする機会は増えても、実際に手を動かすまでは意外と距離があります。

その「距離感」の中身を探ると、次のような思いが見えてきます。

- Claude Codeを試してみたいが、何が作れるのか分からない
- コードが書けない自分でも本当に使いこなせるのか不安
- 実際に使った人のリアルな体験談・つまずいたポイントを知りたい

<strong>この距離は、「作りたいもの」を一つ決めて対話で指示するだけで縮まります。コードが書けなくても形になります。</strong> 私は2週間で投資シミュレーターとブログを立ち上げ、コードは1行も書きませんでした。
...
静的サイトジェネレーター（Markdownなどの素材ファイルから自動的にHTMLサイトを組み立てる仕組み）については、自分では決めていません。「個人ブログを作りたい。どういうサービス・選択肢があるか調べて」とClaude Codeに相談しました。すると **「Claude Codeとの相性・管理のシンプルさ・無料ホスティングとの組み合わせを考えると、Hugoが良い」** というアドバイスが返ってきました。Hugoは、この静的サイトジェネレーターの代表的なツールのひとつです。テーマはPaperMod、ホスティングはCloudflare Pages（無料）という構成も、同じ会話の中で決まっています。正直なところ、「テーマ」という言葉の意味自体、このブログを作る中でClaude Codeに教わって知りました。
...
ここから先は、ブログの裏側のやや技術的な話になります。細部が分からなくても大丈夫です。「非エンジニアでもここまで任せられるのか」という空気感だけ持ち帰ってもらえれば十分です。
...
Hugoの働きを補足すると、「Markdownファイル（.mdという拡張子のテキストファイル）を書くと、自動的にHTMLのWebサイトに変換してくれるツール」です。Claude Codeでブログ原稿を書くとMarkdownファイルが生成されるので、それをブログの形に変換してくれる仕組み、とイメージしてもらえれば近いと思います。
...
解決策は**Wrangler CLI（Cloudflareの公式CLIツール）を使った直接アップロード**です。ブラウザ上の連携をあきらめ、ターミナルから`npx wrangler pages deploy`でサーバに送信する方法に切り替えました。CLIからの操作は素直に動き、以降はこちらを使っています。
...
この記事を読んで「自分も試してみたい」と思った方は、まず「作りたいもの」を一つだけ決めて、Claude Codeに話しかけてみてください。家計の集計表でも、旅行の予定表でも構いません。対話で指示するだけで形になる——この記事で紹介した全工程の出発点は、その最初のひと言です。

このブログで実際に作ったシミュレーターは、こちらから無料で使えます。自分の条件を入れて、一度動かしてみてください。
...
- [Claude Designでブログのデザインを刷新した話 — PaperMod標準から脱却するまで](/posts/blog-redesign-with-claude-code/)
- [Cloudflare Pagesでブログを公開するまでの仕組みを整理してみた——Draft確認から投稿予約まで](/posts/cloudflare-pages-blog-publish-flow/)
- [Xの自動投稿で苦労した話——GitHubとCloudflareで作る定時投稿の仕組み](/posts/x-auto-post-cloudflare-workers/)
