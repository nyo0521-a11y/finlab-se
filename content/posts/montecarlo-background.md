...
金融系ITエンジニアとして仕事をしていると、「数字は正直に見る」癖がつきます。システム障害でもビジネス判断でも、感覚ではなく数値とロジックで判断する。そのくせを、自分の資産形成にも適用してみたのがこの記事の出発点です。

FIREの取崩しでよく語られる「4%ルール」を調べ直したとき、私はある事実に気づいて少し驚きました。<strong>「成功した98%のケースでは、資産が取り崩しを上回って増え続けていた」</strong>という事実です。

守るべきリスクを恐れるあまり、享受できるはずのリターンを捨てていないか。そう考え始めたのが、[取崩しシミュレーターを自作する](/posts/montecarlo-tool-usage/)きっかけになりました。本記事では、その背景にある考え方を整理します。
