...
このブログには「予約投稿」の機能があります。記事ファイルの冒頭に公開日時を書いておくと、その日時まで本番サイトに表示されない仕組みです（毎時0分の自動ビルドで公開）。このブログの公開フローについては[別記事で詳しく解説しています](/posts/cloudflare-pages-blog-publish-flow/)。
...
本番公開フォルダにブログ記事が追加されたとき、GitHub Actionsのワークフローが記事ファイルの冒頭に書かれた公開日時を確認します。公開日時が未来の場合はキュー登録をスキップし、「公開日時に達したときに自動で登録される」とログに記録します。

**② 毎時0分の自動ビルド：公開になった記事を検知してエンキュー**

毎時0分の自動ビルド処理は、サイトのビルドが完了した後に本番公開フォルダ全体をスキャンします。公開日時が現在時刻以前の記事を見つけたら投稿待ちリストへ追記します。このキュー登録プログラムは投稿済み履歴ファイルも確認するため、すでにXに投稿した記事が再びキューに積まれることはありません。

{{< figure src="/images/x-autopost-enqueue-flow.png" alt="ブログ公開からX投稿までの流れ：push後に公開日時チェック→cronビルドで検知→キュー登録→投稿の7ステップ" caption="pushした時点ではスキップし、記事が実際に公開されたタイミングでキューに登録される" >}}

この仕組みにより、「記事が公開される前にXへの告知が出てしまう」という事態を防ぎつつ、公開後は自動でXへの投稿が予約されます。

### 実際に起きた失敗：過去記事が一括エンキューされた

cronによるエンキューを導入した直後、**ダッシュボードを見たら過去記事が数十件、一気にXの告知キューに入っていた**という事態が発生しました。

原因はcronのスキャン範囲にありました。

{{< figure src="/images/x-autopost-bulk-enqueue-bug.png" alt="一括エンキューが発生した原因：全記事スキャン→全記事が条件を満たす→未投稿と判定→数十件が一括エンキュー" caption="投稿済み履歴ファイルに記録がなかった過去記事がすべて「未投稿」と判断された" >}}

投稿スクリプトには「キューにある」「投稿済みhistoryにある」記事を二重エンキューしない仕組みがあります。ただし、この仕組みを導入する以前の記事はhistoryに記録がなく、すべて「未投稿」と扱われてしまいました。

**対処として2点修正しました。**

まず、キューに積まれた記事を手動でクリアしました。次に、cronのエンキュー条件を変更しました。

{{< figure src="/images/x-autopost-time-filter.png" alt="エンキュー条件の修正前後：修正前は全過去記事が対象、修正後は直近2時間以内のみ対象" caption="時間窓を設けることで、過去記事の一括エンキューを防ぐ" >}}
...
140字を超えそうな場合はdescriptionが自動的に短縮されますが、句読点の位置で自然に切れるようルールが組み込まれています。別途X用の文章を用意する必要はなく、**ブログ記事に丁寧なdescriptionを書く習慣が、そのままXの投稿品質に反映される**という仕組みです。
...
- 過去記事が数十件、一括でX投稿キューに積まれる → 時間窓フィルタの追加
...
- **実際の失敗**：仕組みを導入した直後に過去記事が数十件、一括でキューに積まれた。全記事をスキャンしていたところに、投稿済み記録がない記事が大量にあったことが原因。直近2時間以内に公開日時が到来した記事のみを対象とするフィルタを追加して解決
