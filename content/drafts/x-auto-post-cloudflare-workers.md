...
このブログでは「予約投稿」の機能があります。記事のMarkdownに `date: 2026-05-01T08:00:00+09:00` と書いておくと、その日時まで本番サイトに表示されない仕組みです（毎時0分のcronビルドで自動公開）。
...
`content/posts/`にファイルが追加されたとき、GitHub Actionsのワークフロー（`x-new-post-enqueue`）がfrontmatterの`date`フィールドを確認します。`date`が未来の場合はキュー登録をスキップし、「公開日時に達したときに自動で登録される」とログに記録します。

**② 毎時0分のcronビルド：公開になった記事を検知してエンキュー**

`deploy.yml`のcronビルドは、本番ビルドが完了した後に`content/posts/`全体をスキャンします。`date`が現在時刻以前の記事を見つけたら`x-queue.yaml`へ追記します。エンキュースクリプトは投稿済み履歴（`data/x-post-history.yaml`）もチェックするため、すでにXに投稿した記事が再びキューに積まれることはありません。
