...
{{< figure src="/images/x-autopost-bulk-enqueue-bug.png" alt="一括エンキューが発生した原因：全記事スキャン→全記事が条件を満たす→未投稿と判定→37件一括エンキュー" caption="historyファイルに記録がなかった過去記事がすべて「未投稿」と判断された" >}}
...
- 過去記事37件が一括でX投稿キューに積まれる → 時間窓フィルタの追加
...
**投稿履歴ファイル（data/x-post-history.yaml）**

この仕組みでは投稿ログをYAMLファイルで管理しています。投稿された内容・時刻・種別（新着/リマインド）を確認できます。「同じ記事が2回投稿されている」「キューにあるはずの記事が投稿されていない」といった異常の検知に使えます。
