...
処理の流れ:
  ...
  3. slot の条件判定:
     ...
     - night   : queue>=1 なら投稿（朝の積み残しを追い上げ。空ならフォールバックが既存記事紹介）
...
def should_post_in_slot(slot: str, queue_size: int) -> tuple[bool, str]:
    ...
    if slot == "night":
        # 朝枠の投稿が失敗・積み残した新着をキューから追い上げ投稿する。
        # キューが空のときはワークフロー側のフォールバックが既存記事紹介を投稿する
        return True, f"night slot posts when queue={queue_size}>=1"
