...
{{< figure src="/images/folder-env-diagram.png" alt="GitHubのフォルダとCloudflare Pagesの環境の対応図" caption="左のフォルダ（GitHub）と右の環境（Cloudflare）は別の場所にある。GitHub Actionsがアップロードのたびに両方を自動でビルド・デプロイする" >}}
...
本番アップしたいときは、確認用フォルダから公開用フォルダへ記事を移動してアップロードするだけです。コピーではなく「移動」なので、同期ずれが起きません。
...
この `robots.txt` は GitHub のフォルダに置いているファイルではなく、<strong>アップロード（push）のたびに GitHub Actions が自動生成して Draft 環境のサイト全体に適用するもの</strong>です。確認用フォルダ・公開用フォルダという「フォルダ単位」の制御ではなく、Draft環境という「サイト丸ごと」が Google 検索対象外になっています。
