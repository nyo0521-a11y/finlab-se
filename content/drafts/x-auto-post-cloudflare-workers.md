...
ここで不整合が生まれます。ブログを本番公開フォルダ（GitHubの`posts`フォルダ）にアップロードした瞬間にXキューへ登録してしまうと、**記事がまだ公開前なのにXへの告知が先に出てしまう**リスクがあります。たとえば翌朝公開予定の記事を夜にアップロードしたとき、その夜のうちにXへお知らせが届いてしまう、という状況です。
