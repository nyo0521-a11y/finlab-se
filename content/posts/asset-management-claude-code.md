...
description: "毎月の資産記録が続かない人へ。MoneyForwardの残高更新→スプレッドシートの資産推移表→貸借対照表Excelという3ステップを、コードを書かずにClaude Codeへの指示だけで回す方法を、金融SEが実際の指示例とともに解説します。"
showToc: true
cover:
...
「資産管理は大事と分かっているけど、毎月の記録作業が面倒で続かない」——家計簿や資産推移の記録に挫折した経験がある方は、決して少なくないはずです。
...
<strong>必要なのは「出力フォーマットとインプットを渡すこと」だけで、あとの手間はClaude Codeが埋めてくれます。</strong> 私はこの発想で月次資産管理の手作業を半分に減らし、「ながら作業」で毎月の記録を続けられるようになりました。

資産管理のアウトプット（スプレッドシート・貸借対照表）はすでに構造が決まっており、毎月同じ場所に数字を入れるだけです。本記事では、私が実際に運用しているMoneyForward更新→スプレッドシート→貸借対照表Excelの3ステップを、Claude Codeへの指示例つきで確認していきます。
...
私が資産の記録を始めたのは2021年頃です。きっかけは家計簿アプリ「MoneyForward Me」（無料版）の仕様変更でした。無料版では資産の推移が**直近1年分しか閲覧できなくなった**ため、「このままでは2014年から蓄積してきた過去のデータが見えなくなってしまう」という危機感から、慌ててMoneyForward内のデータをGoogleスプレッドシートに取り出し、以降は月次で手動記録を続けています。
...
それでも、月次作業のために30分間机に向かい続ける必要がなくなったことの価値は大きいと感じています。トークン切れで一時的に待つことがあっても、「ながら作業」で続けられる仕組み自体は変わらず機能しています。

---

## これからClaude Codeを試したい方へ

ここまで紹介した3ステップは、すでにスプレッドシートや貸借対照表を運用している人向けの話です。「まだそんな管理表は持っていない」という方も、心配はいりません。

Claude Codeは[Anthropic公式サイト](https://claude.com/product/claude-code)からアカウント登録すれば使えます（月額プランが必要です）。契約が済んだら、いきなり本記事のような3ステップを目指す必要はありません。まずは次のような、ごく簡単な一言から試してみてください。

> 「銀行口座と証券口座の残高を毎月記録するシンプルなスプレッドシートを、Googleスプレッドシートで一緒に作ってほしい。項目は『日付』『口座名』『残高』の3つだけでいい。」

この一言だけで、Claude Codeが管理表の雛形を作るところから手伝ってくれます。慣れてきたら、本記事のようにアセットアロケーションの集計や貸借対照表への展開を少しずつ足していけば十分です。
