---
title: "クレカ決済はなぜ0.1秒で完了する？5社が動く処理フローを金融SE実務者が解説"
date: 2026-04-17T10:00:00+09:00
lastmod: 2026-08-16T00:00:00+09:00
draft: false
categories: ["金融の仕組み"]
tags: ["クレジットカード", "金融インフラ", "金融SE"]
description: "ECの「購入」ボタンから0.1秒で決済が完了するのはなぜか。加盟店・PSP・アクワイアラ・国際ブランド・イシュアが連携するオーソリ処理を5ステップで図解。イオンカード不正利用の背景にある「フロアリミット」縮小の動きや3Dセキュアの仕組みも解説します。"
showToc: true
cover:
  image: "/images/thumb-credit-card-payment-flow.png"
...
その0.1秒の内側では、<strong>加盟店・PSP・アクワイアラ・国際ブランド・イシュアという5つの組織がリレー形式で承認フローを回しています。</strong> この5者の役割を押さえれば、決済の速さも不正利用の仕組みも一気に見通せます。

本記事では1回の決済をこの5ステップに分解したうえで、3Dセキュアの仕組みや、イオンカードの不正利用事件で実際に何が起きたのかまで、実例を交えて確認していきます。
...
カード会員を含めると、1回の決済にはこの6者が関わっています（処理を担う組織は先述の5社）。なお、PSPを持たない加盟店（大手EC等）は、直接アクワイアラと接続するケースもあります。
...
3Dセキュアの「3D」は、3つのドメイン（イシュアドメイン・アクワイアラドメイン・相互運用ドメイン）が連携することに由来します。イシュアドメインでカード発行会社がカード会員本人を認証し、アクワイアラドメインで加盟店側の取引を認証し、両者を国際ブランドが担う相互運用ドメインが仲介します。2021年以降に普及している**3Dセキュア2.0**（EMV 3-DS）は、旧バージョンに比べてユーザー体験が大きく改善されました。
...
## 5. フロアリミット縮小の動き：少額決済のオーソリ省略とイオンカード不正利用の教訓

ここまで説明してきた「イシュアへのオーソリ（与信確認）」ですが、実はすべての取引で必ず行われるわけではありません。その省略運用「フロアリミット」の仕組みと、なぜ今その適用範囲が縮小しつつあるのかを見ていきます。

### オーソリ省略の仕組み：なぜ存在したのか

かつてのクレジットカード決済では、**少額取引についてはイシュアへのオーソリを省略できる運用**が一部で存在していました。これを「**フロアリミット（Floor Limit）**」と呼びます。
...
2024年にイオンカードで多発した不正利用では、フィッシング詐欺等で入手したカード情報がスマートフォンのiD（Apple Pay）に登録されていました。その状態で**機内モードにより通信を遮断**し、少額のオフライン決済を繰り返す手口が使われたとみられています（ケータイ Watch・ITmedia等の報道による）。攻撃のパターンを整理すると、次のような流れです。
...
こうした事案を受け、現在ではフロアリミットによるオーソリ省略は縮小が進んでいるとみられます。具体的な廃止時期や適用範囲はブランド・加盟店ごとに異なるため、詳細は各カード会社・決済代行会社の公表情報をご確認ください。縮小が進んでいる主な理由は次の2点です。
...
先ほど「3Dセキュア2.0の役割」の章で説明したとおり、3Dセキュアは「オンライン決済の本人認証」を強化する仕組みです。一方でオーソリ省略問題への対応は、その前段階——「そもそもイシュアに問い合わせを届かせる」という部分の設計に関わる問題だったと言えます。

> **ここまでのポイント**
> - 少額決済は「フロアリミット」によりイシュアへの問い合わせが省略されることがあった
> - イオンカードの不正利用は、この仕組みと機内モードの組み合わせで不正検知・停止命令の両方をすり抜けた
> - 通信インフラの整備とブランドの方針変更で、現在はオーソリ省略が縮小する方向にある
...
- オーソリゼーション（与信確認）とクリアリング（売上確定）は別処理で、お金が動くタイミングが違う
- 3Dセキュア2.0は毎回パスワードを求めるのではなく、リスクベース認証で必要なときだけ本人確認を挟む仕組み
- オーソリを省略できる「フロアリミット」という運用が不正利用の温床になった事案があり、現在は縮小が進んでいる
- 決済に関わる組織はカード会員を含めて6者。還元率や不正検知の精度はイシュアのビジネスモデルに左右される

次にできることとして、まずはご自身が使っているカードの「3Dセキュア対応状況」と「不正利用時の補償内容」を、カード会社の公式サイトで確認してみてください。仕組みを知ったうえで見ると、単なる規約確認とは違った視点で読めるはずです。
...
購入ボタンを押した瞬間にイシュアが行う「与信確認」のことです。カードが有効か・利用枠が足りるか・不正の疑いがないかをチェックしますが、この時点ではまだお金は動きません。実際の支払い確定は後日の「クリアリング」で行われます。

### Q. なぜ3Dセキュアの認証画面が出るときと出ないときがあるのですか？

2.0以降は「リスクベース認証」が採用されており、普段と同じデバイス・金額の取引は自動でリスクが低いと判定され、認証画面が省略されます。逆に高額決済や普段と違う端末では本人確認が求められます。

### Q. イオンカードの不正利用はなぜ機内モードで防げなかったのですか？

少額決済ではイシュアへの問い合わせを省略する「フロアリミット」という仕組みが使われることがあり、さらに機内モード（通信遮断）でオフライン決済を繰り返すと、不正検知スコアリングもカード停止命令も届かなくなるためです。
