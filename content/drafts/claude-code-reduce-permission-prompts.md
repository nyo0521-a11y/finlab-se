...
**設定ファイルの存在を調べたのではなく、Claude Code に相談したら Claude Code 自身が答えを教えてくれた**——これが実態です。本記事ではその設定内容をそのままプロンプトとして紹介します。
...
**設定ファイルを自分で調べて手書きする必要はありません。**

Claude Code はファイルの作成・編集も自分で行います。「こういう設定を書いて」とプロンプトを渡せば、適切な場所に設定ファイルを作ってくれます。
