...
これはGoogleなどの検索エンジンのクローラー（自動巡回プログラム）に対して「このサイト全体を読み込まないでください」と伝える記述です。この指示に従ったクローラーはページをインデックス（検索データベースへ登録）しません。

この `robots.txt` は GitHub のフォルダに置いているファイルではなく、<strong>アップロードのたびに GitHub Actions が自動生成して Draft 環境のサイト全体に適用するもの</strong>です。確認用フォルダ・公開用フォルダという「フォルダ単位」の制御ではなく、Draft環境という「サイト丸ごと」が Google 検索対象外になっています。
...
- 公開日時が現在時刻より<strong>未来</strong>の場合：Hugoが変換する際に除外され、本番に出ない
...
5. アップロードを受けてCloudflare Pagesが本番サイトを変換・Web公開
6. 記事が本番に公開される
...
- GitHubのmainブランチへのアップロードで、Draftと本番の両方が自動で変換・Web公開される
